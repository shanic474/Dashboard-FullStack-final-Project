...
import DeleteDataInDB from "../../components/hooks/DeleteDataInDB.jsx";

const UsersRow = ({ user }) => {
  const setSelectedUser = useSelectionsStore((s) => s.setSelectedUser);
    ...
    <tr className="hover:bg-[#1a1a1a]/50 transition-colors">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm">
        <div className="flex space-x-2">
          ...
          {/* DELETE */}
          <button
            className="p-2 bg-red-500/10 border border-red-500/30 text-red-400 rounded-sm hover:bg-red-500/20 transition-all"
            title="Delete"
            onClick={() => DeleteDataInDB(user._id || user.id, "user")}
          >
            <svg
              className="w-4 h-4"
              fill="none"
